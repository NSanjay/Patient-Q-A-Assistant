...
- Fallback, clarification, and injection-blocked answers are excluded from conversation history passed to subsequent turns, preventing poisoned context from affecting future responses. This is a deliberate security tradeoff - it means pronoun resolution may fail after an unanswerable question, requiring the user to re-state the patient's full name.
...
- **Known edge case:** If a patient is successfully resolved but the question is unanswerable (data not in schema), the fallback response is excluded from history. A subsequent pronoun follow-up ("what is his DOB?") loses patient context and returns Medium confidence fallback. Users should re-state the full patient name after any fallback response.
